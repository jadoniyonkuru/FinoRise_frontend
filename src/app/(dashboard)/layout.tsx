      ...
      <SidebarInset>
        <header className="h-20 flex items-center justify-between px-8 border-b border-black/5 dark:border-white/5 sticky top-0 bg-background/80 backdrop-blur-md z-40">
          <div className="flex items-center gap-6">
            <SidebarTrigger className="h-10 w-10 text-secondary" />
            <div className="relative hidden md:block">
              <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-muted-foreground" />
              <Input 
                placeholder="Search modules, simulations..." 
                className="w-96 pl-12 h-12 glass border-black/20 focus-visible:ring-2 focus-visible:ring-primary/50 text-base text-secondary" 
              />
            </div>
          </div>
          ...
          <div className="flex items-center gap-6">
            <div className="hidden sm:flex items-center gap-3 px-4 py-2 rounded-full glass border-black/10">
              ...
              <span className="text-sm font-bold tracking-tight text-secondary">1,240 XP</span>
            </div>
            <ModeToggle />
                    ...
                    {[
                      ...
                      <button key={i} className="flex gap-4 p-4 text-left hover:bg-primary hover:text-white transition-all border-b border-black/5 last:border-0 group">
                        ...
                        <div className="space-y-1 flex-1">
                          ...
                          <p className="text-[10px] text-muted-foreground group-hover:text-white/70 leading-relaxed transition-colors">{n.desc}</p>
                          <p className="text-[9px] text-muted-foreground/40 font-bold uppercase tracking-wider group-hover:text-white/30 transition-colors">{n.time}</p>
                        </div>
                      </button>
                    ))}
                  </div>
